...
In the trigger below you first want to create a set of unique Opportunity Ids that are in the trigger context. You then use those Ids to query and create a map with the Opportunity Id as the key and the Opportunity object as the value. Then when you process your <code>Sales_Order__c</code> records you can access the Opportunity object from the map by its Id (line 14).
